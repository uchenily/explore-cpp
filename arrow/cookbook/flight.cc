...
public:
    ...
    GetFlightInfo(const arrow::flight::ServerCallContext &,
        ...
        ARROW_ASSIGN_OR_RAISE(auto file_info,
                              FileInfoFromDescriptor(descriptor));
    ...
    DoGet(const arrow::flight::ServerCallContext &,
        ...
        ARROW_ASSIGN_OR_RAISE(auto owning_reader,
                              arrow::RecordBatchReader::Make(std::move(batches),
                                                             table->schema()));
...
private:
    ...
    MakeFlightInfo(const arrow::fs::FileInfo &file_info) {
        ...
        return arrow::flight::FlightInfo::Make(*schema,
                                               descriptor,
                                               {endpoint},
                                               total_records,
                                               total_bytes);
    ...
    FileInfoFromDescriptor(const arrow::flight::FlightDescriptor &descriptor) {
        if (descriptor.type != arrow::flight::FlightDescriptor::PATH) {
            return arrow::Status::Invalid(
                "Must provide PATH-type FlightDescriptor");
        } else if (descriptor.path.size() != 1) {
            return arrow::Status::Invalid(
                "Must provide PATH-type FlightDescriptor with one path "
                "component");
        }
...
arrow::Status TestPutGetDelete() {
    ...
    auto root
        = std::make_shared<arrow::fs::SubTreeFileSystem>("./flight_datasets/",
                                                         fs);
    ...
    ARROW_RETURN_NOT_OK(
        reader->GetRecordBatchReader(row_groups, &batch_reader));
    int64_t batches = 0;
    while (true) {
...
//     client_options.generic_options.emplace_back(
...
//     arrow::flight::Location server_location;
//     ARROW_ASSIGN_OR_RAISE(server_location,
//                           arrow::flight::Location::ForGrpcTcp("0.0.0.0",
//                           5000));
//
//     arrow::flight::FlightServerOptions options(server_location);
//     auto server = std::unique_ptr<arrow::flight::FlightServerBase>(
...
//     auto client_channel
//         = grpc::CreateChannel("0.0.0.0:5000",
