...
void StartRecipe(const std::string &recipe_name) {
    if (!current_recipe.empty()) {
        FAIL() << "Attempt to start a recipe " << recipe_name
               << " but the recipe " << current_recipe
               << " has not been marked finished";
    }
...
arrow::Result<std::shared_ptr<arrow::Table>> MakeEmptyRecipesTable() {
    ...
    std::shared_ptr<arrow::RecordBatch> batch = arrow::RecordBatch::Make(
        schema,
        0,
        {MakeEmptyStringArray(), MakeEmptyStringArray()});
    return arrow::Table::FromRecordBatches({batch});
}

arrow::Result<std::shared_ptr<arrow::Table>>
ReadRecipeTable(const std::shared_ptr<arrow::io::RandomAccessFile> &in_file) {
...
arrow::Result<std::shared_ptr<arrow::Table>>
LoadExistingRecipeOutputTable(const std::string &filename) {
...
arrow::Result<std::shared_ptr<arrow::Table>>
MergeRecipeTables(const std::shared_ptr<arrow::Table> &old_table,
    ...
    std::shared_ptr<arrow::RecordBatch> batch
        = arrow::RecordBatch::Make(schema,
                                   names_arr->length(),
                                   {names_arr, outputs_arr});
    return arrow::Table::FromRecordBatches({batch});
}

bool HasRecipeOutput() {
    return test_names_builder.length() > 0;
}
