...
public:
    void SetUp() override {
        airquality_partitioned_dir_ = std::filesystem::temp_directory_path()
                                      / "cookbook_cpp_airquality";
    ...
    const std::string &airquality_basedir() {
        return airquality_partitioned_dir_;
    }

private:
    void
    WritePartitionedAirQuality(const std::shared_ptr<arrow::Table> &airquality,
                               std::shared_ptr<arrow::fs::FileSystem> fs) {
        ...
        std::shared_ptr<arrow::Schema> partitioning_schema
            = arrow::schema({arrow::field("Month", arrow::int32()),
                             arrow::field("Day", arrow::int32())});
        std::shared_ptr<arrow::dataset::PartitioningFactory>
        ...
        write_options.file_write_options
            = parquet_format->DefaultWriteOptions();
    ...
    ReadInAirQuality(arrow::fs::FileSystem *fs) {
...
arrow::Status DatasetRead(const std::string &airquality_basedir) {
    ...
    // format such as Arrow-IPC files or CSV files or we could customize the
    // parquet format with additional reading & parsing options.
    ...
    // dataset factory to infer the partitioning schema from the filenames.  All
    // we need to specify is the flavor of partitioning which, in our case, is
    // "hive".
    //
    // Alternatively, we could manually create a partitioning scheme from a
    // schema.  This is typically not necessary for hive partitioning as
    ...
    // files and may scan some file metadata in order to determine the dataset
    // schema.
    ...
    return DumpRecipeOutput("TEST_DatasetRead.txt");
}
